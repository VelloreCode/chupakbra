...
export class ScrapingQueue {
  ...
  private async executeJob(job: ScrapingJob): Promise<any> {
    ...
    try {
      ...
      // Handle different job types - default to price_update for backward compatibility
      const jobType = job.type || 'price_update';
      switch (jobType) {
        case 'price_update':
...
// Force restart queue when module is imported  
export function forceRestartQueue(): ScrapingQueue {
  if (globalQueue) {
    globalQueue.close();
    globalQueue = null;
  }
